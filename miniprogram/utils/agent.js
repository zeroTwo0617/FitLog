...
function persistSession(sessionId, messages, context) {
  const serverDate = agentRepo.serverDate()
  const data = {
  ...
  if (sessionId) {
    return agentRepo.updateSession(sessionId, data).then(() => sessionId)
  }
  return agentRepo.createSession({
    data: Object.assign({}, data, { createdAt: serverDate, title: '训练助手会话' })
