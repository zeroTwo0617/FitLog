...
- 增加“我的 > 应用 > 设置”主题入口，支持浅色/深色主题切换和本地持久化。
- 主题切换同步页面底色、微信顶栏、自定义底部导航和主题图标。
- 参照 `design/fitlog-ui-prototype.html` 调整全局字体层级：基础字号提升至 `28rpx`，页面标题提升至 `58-62rpx`，正文、按钮、输入框、动作名和 Agent 消息同步放大。
- 底部导航文字保持 `18rpx`，避免导航高度变化和内容区域跳动。

### 文档

- 新增 `docs/architecture/ui-design-system.md`，记录字号、主题、样式职责和验收清单。
- 根目录开发文档、README、文档索引和 V1 发布清单同步更新。

### 验证

- JavaScript、JSON、WXSS 花括号和 `git diff --check` 静态检查通过。
- 未操作微信开发者工具，未提交私有配置和设计目录资源。
